...
disttoscreen                = 65; % distance to eyetracker in cm. We assume distance to screen is the same; might be slight overestimation though
rad2deg                     = @(x) x/pi*180;
...
folders.data                = '../data/TX300 Kenneth'; % folder in which data is stored (each folder in folders.data is considered 1 subject)
...
fid = fopen([folders.output fs 'allfixationsKenneth.txt'],'w');
...
[file,nfile] = FileFromFolder([folders.data],[],'hdf5');

for f = 1:nfile
    
    %pre-allocate name for saving results to
    savefile = [folders.output fs file(f).name(1:end-4)];
    
    %% IMPORT DATA
    
    fprintf('Importing and processing %s \n',file(f).name)
    
    [timestamp,llx,lly,rrx,rry] = importTobiiTX300EDQ([folders.data fs file(f).name],[xres yres],missingx,missingy);
    ...
    [finalweights_left,stopped] = twoClusterWeighting2(llx,lly,llmiss,windowtime,steptime,freq,maxerrors);
    ...
    [finalweights_right,stopped] = twoClusterWeighting2(rrx,rry,rrmiss,windowtime,steptime,freq,maxerrors);
    ...
    for g=1:numel(fixstart)
        fprintf(fid,'%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%s\t%s\n',[fixstart(g) fixend(g) fixdur(g) xmedian(g) ymedian(g) flankdataloss(g) fracinterped(g) cutoff, RMSxy(g), BCEA(g), fixRangeX(g), fixRangeY(g)],'',file(f).name(1:end-4));
    end
