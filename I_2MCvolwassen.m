...
folders.data                = '../data/TX300 Volwassen'; % folder in which data is stored (each folder in folders.data is considered 1 subject)
...
fid = fopen([folders.output fs 'allfixationsVolwassen.txt'],'w');
...
for e = 1:nfold     % 3, 16 for one sample peak test
    ...
    for f = 1:nfile
        ...
        [timestamp,llx,lly,rrx,rry] = importTobiiTX300Volwassen([folders.data fs fold(e).name fs file(f).name],1,[xres yres],missingx,missingy);
