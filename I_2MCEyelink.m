...
freq                        = 500; % sampling frequency of data (check that this value matches with values actually obtained from measurement!)

% VISUAL ANGLE STUFF
diagcm                      = sqrt(53.2^2+29.9^2);
diagpix                     = sqrt(xres^2+yres^2);
pixpercm                    = diagpix/diagcm;
disttoscreen                = 55; % distance to eyetracker in cm. We assume distance to screen is the same; might be slight overestimation though
rad2deg                     = @(x) x/pi*180;
...
windowedgeInterp            = 0.02; % amount of data (s) at edges used for interpolation
...
cutoffstd                   = 3; % number of standard deviations above mean k-means weights will be used as fixation cutoff
maxMergeDist                = 8; % maximum Euclidean distance in pixels between fixations for merging
maxMergeTime                = 30; % maximum time in ms between fixations for merging
minFixDur                   = 30; % minimum fixation duration after merging, fixations with shorter duration are removed from output

%% SET-UP FOLDERS

folders.data                = '../data/Eyelink 1000'; % folder in which data is stored (each folder in folders.data is considered 1 subject)
...
fid = fopen([folders.output fs 'allfixationsEyelink.txt'],'w');
...
for e = 1:nfold     % 3, 16 for one sample peak test
    ...
    for f = 1:nfile
        ...
        [timestamp,llx,lly,rrx,rry] = readEyelink([folders.data fs fold(e).name fs file(f).name],[xres yres],missingx,missingy);
        ...
        interpwins   = findInterpWinsMaxDisp(xpos,ypos,missing,windowtimeInterp,windowedgeInterp,freq,maxdisp);
        llinterpwins = findInterpWinsMaxDisp(llx ,lly ,llmiss ,windowtimeInterp,windowedgeInterp,freq,maxdisp);
        rrinterpwins = findInterpWinsMaxDisp(rrx ,rry ,rrmiss ,windowtimeInterp,windowedgeInterp,freq,maxdisp);
        
        % spline interpolate and replace values
        fprintf('Cubic spline interpolation\n');
        [xpos,ypos,missingn]= windowedSpline(xpos,ypos,missing,  interpwins);
        [llx ,lly ,llmiss]  = windowedSpline(llx ,lly ,llmiss ,llinterpwins);
        [rrx ,rry ,rrmiss]  = windowedSpline(rrx ,rry ,rrmiss ,rrinterpwins);
        ...
        [finalweights,stopped] = twoClusterWeighting2Variable(xpos,ypos,missingn,downsamples,windowtime,steptime,freq,maxerrors);
...
%         pause
        ...
        [finalweights_left,stopped] = twoClusterWeighting2Variable(llx,lly,llmiss,downsamples,windowtime,steptime,freq,maxerrors);
        ...
        [finalweights_right,stopped] = twoClusterWeighting2Variable(rrx,rry,rrmiss,downsamples,windowtime,steptime,freq,maxerrors);
